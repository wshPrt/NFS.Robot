...
namespace NFS.Commons.VlcWPF
{
    public class VlcPlayer
    {
        public class VlcPlayerBase
        {
            ...
            private static IntPtr libvlc_media_player_;
            ...
            {
                LibVlcAPI.libvlc_video_set_aspect_ratio(libvlc_media_player_, ratio);
            }
            ...
            {
                var path = AppDomain.CurrentDomain.BaseDirectory + "\\temp";
                if (System.IO.Directory.Exists(path) == false)
                    Directory.CreateDirectory(path);
                var tempPath = Path.Combine(path, DateTime.Now.ToString("yyyyMMddHHmmssff") + ".png");
                var result = LibVlcAPI.libvlc_video_take_snapshot(libvlc_media_player_, 0, tempPath, 0, 0);
                Console.WriteLine(result);
                if (File.Exists(tempPath))
                {
                    return GetImageByPath(tempPath);
                }
                return null;
            }
            ...
            public Image GetImageByPath(string tempPath)
            {
                try
                {
                    byte[] bytes = null;
        ...
        internal static class LibVlcAPI
        {
            ...
            [DllImport("libvlc", CallingConvention = CallingConvention.Cdecl, ExactSpelling = true)]
            [SuppressUnmanagedCodeSecurity]
            public static extern IntPtr libvlc_media_player_new_from_media(IntPtr libvlc_media_player);
            ...
            [DllImport("libvlc", CallingConvention = CallingConvention.Cdecl, ExactSpelling = true)]
            [SuppressUnmanagedCodeSecurity]
            public static extern int libvlc_video_take_snapshot(IntPtr p_mi, int num, string psz_filepath, int i_width, int i_height);
